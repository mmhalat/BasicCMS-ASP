...
<% 
	Function TR(strGelen)
		...
		TR = Replace(TR, "Ö", "O")
	End Function
sql="select * from hadis where isim like '%"&request("huara")&"%' or ac like '%"&request("huara")&"%' order by isim asc"
rs.Open sql, kon, 1, 3
Set arailmihal = Server.CreateObject ("ADODB.RecordSet")
sql = "select * from hadis order by id"
...
<% Do While not rs.eof %>
<tr>
<td><a target="_blank" href="hadis_bankasi_oku.asp?id=<% =arailmihal("id")%>"><%=TR(rs("isim"))%></a></td>
</tr>
<% 
arailmihal.movenext
rs.movenext
