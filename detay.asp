...
<body>
  ...
  <table width="983" border="0" align="center" cellpadding="0" cellspacing="0" cols="2" bgcolor="#FEFEFE">
    ...
    <tr>
      ...
      <td width="546" valign="top" background="images/bg.jpg" class="style4"><table width="537" border="0" cellspacing="0" cellpadding="0"><%id = request.querystring("id")
				set rs = server.createobject("ADODb.recordset")
				sql="select * from bb where id="&id&""
				rs.open sql,baglanti,1,3
				if rs.eof then
				Response.Write "<script>alert('Bu ID bulunamadı. Silinmiş yada hiç eklenmemiş olabilir !!');</script>"%>
        <tr>
          <td colspan="2">Soru : <%=rs("soru")%></td>
          </tr>
        <tr>
          <td colspan="2">Cevap : <%=rs("cevap")%></td>
          </tr>
        <tr>
          <td colspan="2">Toplam <%=rs("hit")%> Defa Okundu </td>
          </tr>
        <tr>
          <td width="267">Ekleyen:<%=rs("ekleyen")%></td>
          <td width="270">Tarih:<%=rs("tarih")%></td>
        </tr>
						<% end if %>
      </table>
